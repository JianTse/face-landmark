...
int MTCNN::init(const string& proto_model_dir) {
...
void MTCNN::GenerateBBox(cv::Mat* confidence, cv::Mat* reg_box,
...
vector<FaceInfo> MTCNN::NextStage(const cv::Mat& image, vector<FaceInfo> &pre_stage_res, int input_w, int input_h, int stage_num, const float threshold) {
	...
	if (batch_size == 0)
		return res;
	cv::Mat* input_layer = nullptr;
	cv::Mat* confidence = nullptr;
	cv::Mat* reg_box = nullptr;
	cv::Mat* reg_landmark = nullptr;
	...
	cv::Mat blob_input = cv::dnn::blobFromImages(inputs, std_val, cv::Size(), cv::Scalar(mean_val, mean_val, mean_val), false);
	...
	case 2: {
		...
		const std::vector< cv::String >  targets_node{ "conv5-2","prob1" };
	...
	case 3: {
		...
		const std::vector< cv::String >  targets_node{ "conv6-2","conv6-3","prob1" };
...
vector<FaceInfo> MTCNN::Detect_mtcnn(cv::Mat& src, const int minSize, const float* threshold, const float factor, const int stage) {
	
	vector<FaceInfo> pnet_res;
	vector<FaceInfo> rnet_res;
	vector<FaceInfo> onet_res;
	if (stage >= 1) {
		pnet_res = ProposalNet(src, minSize, threshold[0], factor);
	}
	if (stage >= 2 && pnet_res.size()>0) {
		...
		for (int iter = 0; iter < size; ++iter) {
			...
			vector<FaceInfo> res = NextStage(src, input, 24, 24, 2, threshold[1]);
			rnet_res.insert(rnet_res.end(), res.begin(), res.end());
		}
		rnet_res = NMS(rnet_res, 0.4f, 'm');
		BBoxRegression(rnet_res);
		BBoxPadSquare(rnet_res, src.cols, src.rows);
