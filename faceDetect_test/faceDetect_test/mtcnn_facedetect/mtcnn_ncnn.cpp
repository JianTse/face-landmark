...
int MTCNN_NCNN::init(const string &model_path)
{
	...
	int ret_param1 = Pnet.load_param(param_files[0].data());
	int ret_bin1 = Pnet.load_model(bin_files[0].data());
	int ret_param2 = Rnet.load_param(param_files[1].data());
	int ret_bin2 = Rnet.load_model(bin_files[1].data());
	int ret_param3 = Onet.load_param(param_files[2].data());
	int ret_bin3 = Onet.load_model(bin_files[2].data());
	if (ret_param1 != 0 || ret_bin1 != 0 ||
		ret_param2 != 0 || ret_bin2 != 0 ||
		ret_param3 != 0 || ret_bin3 != 0)
	{
		return 0;
	}		
...
void MTCNN_NCNN::setMinFace(int minSize){
	minsize = minSize;
}

void MTCNN_NCNN::setThreshold(std::vector<float>& threshs)
{
	threshold[0] = threshs[0];
	threshold[1] = threshs[1];
	threshold[2] = threshs[2];
}

void MTCNN_NCNN::generateBbox(ncnn::Mat score, ncnn::Mat location, std::vector<Bbox>& boundingBox_, float scale){
    const int stride = 2;
...
void MTCNN_NCNN::nmsTwoBoxs(std::vector<Bbox>& boundingBox_, std::vector<Bbox>& previousBox_, const float overlap_threshold, std::string modelname)
{
...
void MTCNN_NCNN::nms(std::vector<Bbox> &boundingBox_, const float overlap_threshold, std::string modelname){
    if(boundingBox_.empty()){
        return;
    }
    sort(boundingBox_.begin(), boundingBox_.end(), cmpScore);
...
void MTCNN_NCNN::refine(std::vector<Bbox> &vecBbox, const int &height, const int &width, bool square){
    if(vecBbox.empty()){
...
void MTCNN_NCNN::extractMaxFace(std::vector<Bbox>& boundingBox_)
{
	if (boundingBox_.empty()) {
		return;
	}
...
void MTCNN_NCNN::PNet(float scale)
{
	//first stage
...
void MTCNN_NCNN::detect(ncnn::Mat& img_, std::vector<Bbox>& finalBbox_){
    img = img_;
    img_w = img.w;
    img_h = img.h;
...
void MTCNN_NCNN::detectMaxFace(ncnn::Mat& img_, std::vector<Bbox>& finalBbox) {
...
float MTCNN_NCNN::checkFace(cv::Mat& img, cv::Rect& faceRect) {	
	...
	cv::Mat detectPatch = img(roi).clone();

	ncnn::Mat ncnn_img;
	if (img.channels() == 1)
	{
		ncnn_img = ncnn::Mat::from_pixels_resize(detectPatch.data, ncnn::Mat::PIXEL_GRAY2RGB, detectPatch.cols, detectPatch.rows, detectPatch.cols, detectPatch.rows);
	}
	else
	{
		ncnn_img = ncnn::Mat::from_pixels_resize(detectPatch.data, ncnn::Mat::PIXEL_BGR2RGB, detectPatch.cols, detectPatch.rows, detectPatch.cols, detectPatch.rows);
	}
	...
	threshs[1] = 0.5;
	threshs[2] = 0.6;
	setThreshold(threshs);
	int minFace = min(faceRect.width, faceRect.height) * 0.25;
	setMinFace(minFace);

	int minSize = minFace * minFace * 1.5;
	...
	for (int i = 0; i < num_box; i++) {
		...
		cv::Rect rect = valid_rect(pt1, pt2, detectPatch.cols, detectPatch.rows);
		if (rect.area() < minSize)
		{
			continue;
		}
		float score = bbox.score;
		cv::rectangle(detectPatch, rect, cv::Scalar(0, 0, 0));
		maxScore = score;

		char info[255];
		sprintf(info, "%f ", score);
		cv::putText(detectPatch, info, cv::Point(10, 50), CV_FONT_HERSHEY_COMPLEX, 0.5, cv::Scalar(0, 0, 255));
	}

	//cv::imshow("patch", detectPatch);
	//cv::waitKey(1);

	/*
	ncnn::Mat in;
	resize_bilinear(ncnn_img, in, 48, 48);
	ncnn::Extractor ex = Onet.create_extractor();
	ex.set_num_threads(4);
	ex.set_light_mode(true);
	ex.input("data", in);
