...
void str2FLdmak(std::vector<std::string>& params, int startId, int endId, std::vector<cv::Point2f>& ldmark)
{
	...
	for (int i = 0; i < pointSize; i++)
	{
		float x = atof(params[startId + i * 2].c_str());
		float y = atof(params[startId + i * 2 + 1].c_str());
		ldmark.push_back(cv::Point2f(x, y));
	}
}
...
std::string  FloatLdmark2Str(std::vector<cv::Point2f>& ldmark)
{
	std::string dstStr = "";
	char buff[255];
	for (int i = 0; i < ldmark.size(); i++)
	{
		std::sprintf(buff, " %f %f", ldmark[i].x, ldmark[i].y);
		dstStr = dstStr + buff;
	}
	return dstStr;
}
...
std::string  IntLdmark2Str(std::vector<cv::Point>& ldmark)
{
	...
	for (int i = 0; i < ldmark.size(); i++)
	{
		std::sprintf(buff, " %d %d", ldmark[i].x, ldmark[i].y);
		dstStr = dstStr + buff;
	}
	return dstStr;
}
...
std::string  eva2Str(std::vector<float>& eva)
{
	std::string dstStr = "";
	char buff[255];
	std::sprintf(buff, " %f %f %f", eva[0], eva[1], eva[2]);
	dstStr = dstStr + buff;
	return dstStr;
}
...
std::string  rect2Str(cv::Rect& rect)
{
	std::string dstStr = "";
	char buff[255];
	std::sprintf(buff, " %d %d %d %d", rect.x, rect.y, rect.width, rect.height);
	dstStr = dstStr + buff;
	return dstStr;
}