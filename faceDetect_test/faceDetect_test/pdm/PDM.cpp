...
void PDM::CalcShape3D(cv::Mat_<float>& out_shape, const cv::Mat_<float>& p_local) const
{
	...
	//cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, p_local_cols, princ_comp_rows, princ_comp_cols, alpha1, (float*)p_local.data, p_local_cols, (float*)princ_comp.data, princ_comp_cols, beta1, (float*)out_shape.data, p_local_cols);
...
void PDM::CalcParams(cv::Vec6f& out_params_global, const cv::Rect_<float>& bounding_box,
...
void PDM::CalcParams(cv::Vec6f& out_params_global, cv::Mat_<float>& out_params_local, const cv::Mat_<float> & landmark_locations, const cv::Vec3f rotation)
{
	...
	for (size_t i = 0; i < 1000; ++i)
	{
		...
		//cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, J.cols, J_w_t.rows, J_w_t.cols, alpha1, (float*)J.data, J.cols, (float*)J_w_t.data, J_w_t.cols, beta1, (float*)Hessian.data, J.cols);

		// Above is a fast (but ugly) version of 
		cv::Mat_<float> Hessian = J_w_t * J + regularisations;
...
void PDM::CalcGlobalParams(cv::Vec6f& out_params_global, cv::Mat_<float>& out_params_local, const cv::Mat_<float> & landmark_locations, const cv::Vec3f rotation)
{
