...
#include "spoof\spoofByModel.h"
#include "spoof\spoofOpticalFlow.h"
#include "spoof\spoofNirDepth.h"
...
int detectVideo()
{
	...
	SpoofOpticalFlow _spoofLK;
	cv::Mat lastGray;
	cv::Mat curGray;

	SpoofNirDepth  _spoofDepthNir;

	std::string  rootDir = "E:/work/data/Anti-spoofing/beadwallet/videoSample/pos/record/";
	std::string  videoFn = rootDir + "2019-07-24-10-02-07.avi";
	//cv::VideoCapture mCamera(videoFn.c_str());
	cv::VideoCapture mCameraBgr(2);
	cv::VideoCapture mCameraNir(1);
	if (!mCameraBgr.isOpened() || !mCameraNir.isOpened()) {
	...
	for (;;) {
		...
		mCameraNir >> frameNir;
		if (frameNir.empty()) break;
		
		cv::Rect bgrRect = cv::Rect(0, 0, frameBgr.cols, frameBgr.rows);
		detectFaceImg(detector, frameBgr, bgrRect, retBgr);
		...
		if (lastGray.cols != curGray.cols || lastGray.rows != curGray.rows)
			lastGray = curGray.clone();
		//
		for (int i = 0; i < retBgr.size(); i++)
		{
			int ret = _spoofLK.update(lastGray, curGray, retBgr[i].rect, retBgr[i].ldmark);
			if (ret == 0)  //假
			{
			...
			cv::rectangle(frameBgr, retBgr[i].rect, color, 2);
		}
		lastGray = curGray.clone();
		...
		{			
			int ret = 0;
			int nirRet = _spoofDepthNir.nir_liveness_detection(detector, frameBgr, frameNir, retBgr[i].rect);
			
			char info[1280];
			std::vector<float> scores;
			scores = spoof.cvCheckFace(frameBgr, retBgr[i].rect);
			//scores = cvSpoof.run(frameBgr, retBgr[i].rect);
			...
			cv::rectangle(frameBgr, retBgr[i].rect, color);		
			cv::putText(frameBgr, info, cv::Point(retBgr[i].rect.x, retBgr[i].rect.y), 1, 1, color);
		}
#endif
		
		cv::imshow("bgr", frameBgr);
		cv::imshow("nir", frameNir);
...
int testSpoof()
{
	...
	for (int i = 0; i < files.size(); i++)
	{
		...
		vector<float> probs = spoof.cvCheckFace(img, rect);
...
void  detectDir()
{
	...
	std::string  dstRootDir = "E:/work/data/Anti-spoofing/beadwallet/videoSample/neg/paint/img/";
