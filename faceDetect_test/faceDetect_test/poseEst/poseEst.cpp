...
static const int  eav_th_front = 12;
static const int  eav_th_low = 12;
static const int  pitch_eav_th_low = 8;
static const int  eav_th_high = 45;
...
cv::Mat CPoseEst::ldmark2mat(const std::vector<cv::Point>& ldmarks)
{
	if (ldmarks.size() < 1)
		return cv::Mat();
...
void CPoseEst::judgeSiglePose(int eav0, int eav1, int eav2, PoseInfo& faceInfo)
{
	//pitch
	int tmp = abs(eav0);
	if ((tmp - pitch_eav_th_low) > 0 && (tmp - eav_th_high) < 0)
	{
		if (eav0 < 0)
		{
			faceInfo.is_pitch_up = true;
		}
		else {
			faceInfo.is_pitch_down = true;
		}
	}
	//yaw
	tmp = abs(eav1);
	if ((tmp - eav_th_low) > 0 && (tmp - eav_th_high) < 0)
	{
		if (eav1 < 0)
		{
			faceInfo.is_yaw_left = true;
		}
		else {
			faceInfo.is_yaw_right = true;
		}
	}
	//roll
	tmp = abs(eav2);
	if ((tmp - eav_th_low) > 0 && (tmp - eav_th_high) < 0)
	{
		if (eav2 < 0)
		{
			faceInfo.is_roll_righ = true;
		}
		else {
			faceInfo.is_roll_left = true;
		}
	}

	//找出最大值
	int index = 0;
	int maxValue = abs(eav0);
	if (abs(eav1) > maxValue)
	{
		index = 1;
		maxValue = abs(eav1);
	}
	if (abs(eav2) > maxValue)
	{
		index = 2;
		maxValue = abs(eav2);
	}
	if (maxValue > eav_th_high)//最大角度太大，表示出错
	{
		faceInfo.pose = -1;
	}
	else if ((index == 0) && (eav0 < 0) && (abs(eav0) > pitch_eav_th_low))
	{
		faceInfo.pose = pitch_up;
	}
	else if ((index == 0) && (eav0 > 0) && (eav0 > pitch_eav_th_low))
	{
		faceInfo.pose = pitch_down;
	}
	else if ((index == 1) && (eav1 < 0) && (abs(eav1) > eav_th_low))//
	{
		faceInfo.pose = yaw_left;
	}
	else if ((index == 1) && (eav1 > 0) && (eav1 > eav_th_low))//
	{
		faceInfo.pose = yaw_right;
	}
	else if ((index == 2) && (eav2 > 0) && (eav2 > eav_th_low))//
	{
		faceInfo.pose = roll_left;
	}
	else if ((index == 2) && (eav2 < 0) && (abs(eav2) > eav_th_low))//
	{
		faceInfo.pose = roll_right;
	}
	else
	{
		faceInfo.pose = -1;
...
int CPoseEst::judgePose(const std::vector<float>& eav, PoseInfo& faceInfo)
{
	if (eav.size() < 1)
	{
		return 0;
	...
	faceInfo.pose = -1;
	faceInfo.is_front = false;
	faceInfo.is_pitch_up = false;
	faceInfo.is_pitch_down = false;
	faceInfo.is_yaw_left = false;
	faceInfo.is_yaw_right = false;
	faceInfo.is_roll_left = false;
	faceInfo.is_roll_righ = false;

	int eav0 = int(round(eav[0] + 3));//pitch
	int eav1 = int(round(eav[1]));//yaw
	int eav2 = int(round(eav[2]));//roll

	if ((abs(eav0) < eav_th_front) && (abs(eav1) < eav_th_front)
		&& (abs(eav2) < eav_th_front))
	{
		faceInfo.pose = front;
	}
	else
	{
		judgeSiglePose(eav0, eav1, eav2, faceInfo);
	}

	return 1;
}

bool CPoseEst::estimateEav(const std::vector<cv::Point>& ldmarks, const int faceWidth, std::vector<float>& eavs)
{
...
int  CPoseEst::facePoseEstimate(std::vector<cv::Point>& ldmark_pts, PoseInfo& poseInfo)
{
	...
	estimateEav(ldmark_pts, poseInfo.face_width, poseInfo.eav);
	judgePose(poseInfo.eav, poseInfo);
	return 1;
}
